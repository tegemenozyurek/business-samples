...
import type { SipProduct } from "@/lib/sip-content";
...
export function SipProductCard({
  item,
  index = 0,
}: {
  item: SipProduct;
...
export function SipProductGrid({
  products,
  className = "",
}: {
  products: readonly SipProduct[];
  className?: string;
}) {
  return (
    <div
      className={`mt-10 grid grid-cols-2 gap-3 sm:gap-5 lg:mt-14 lg:grid-cols-3 ${className}`}
    >
      {products.map((item, index) => (
        <SipProductCard key={item.name} item={item} index={index} />
      ))}
    </div>
  );
}
